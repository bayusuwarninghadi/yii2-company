...
class Confirmation extends ActiveRecord
{
    ...
    public function checkTransaction($attribute)
    {
        if (!$this->hasErrors()) {
            if ($this->transaction === null) {
                $this->addError($attribute, Yii::t('app', 'Transaction Id Not Found'));
            } else {
                switch ($this->transaction->status) {
                    case (int)Transaction::STATUS_REJECTED:
                        $this->addError($attribute, Yii::t('app', 'Transaction Rejected'));
                        break;
                    case (int)Transaction::STATUS_CONFIRM:
                        $this->addError($attribute, Yii::t('app', 'Transaction Was Approved'));
                        break;
                    case (int)Transaction::STATUS_DELIVER:
                    case (int)Transaction::STATUS_DELIVERED:
                        $this->addError($attribute, Yii::t('app', 'Transaction Was Deliver'));
                        break;
                }
            }
        }
    ...
    public function attributeLabels()
    {
        return [
            ...
            'name' => Yii::t('app', 'Bank User Name'),
            'amount' => Yii::t('app', 'Transfer Amount'),
            'image' => Yii::t('app', 'Payment Struck'),
            'note' => Yii::t('app', 'Leave Me Some Note'),
            'created_at' => Yii::t('app', 'Transfer Date'),
        ];
    }
