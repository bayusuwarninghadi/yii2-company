...
<div class="media" title="<?=Html::decode($model->product->subtitle)?>">
    <?=Html::a(Html::img($model->product->image_url), ['/product/view', 'id' => $model->product_id],['class' => 'media-left'])?>
    <div class="media-body">
        ...
        <div class="media-content">
            <?php if ($model->product->discount) :?>
                ...
                <?=Html::tag('div', Yii::$app->formatter->asCurrency($model->product->price * (100 - ($model->product->discount)) / 100 * $model->qty));?>
            <?php else :?>
